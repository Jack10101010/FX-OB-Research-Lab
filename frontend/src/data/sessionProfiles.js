...
// 4-LAYER P0 — cohort + entry-key PRIMITIVES now live in the neutral cohortKeys.js
// (no overlay/store/React). They are imported here for internal use AND re-exported
// below so every existing `from "./sessionProfiles"` import keeps working unchanged.
import {
    SESSIONS, SESSION_KEYS, CELLS, CELL_KEYS,
    canonicalSession, tradeSessionRaw, canonicalDirection, canonicalStructure,
    tradeCellKey, cohortOf, thresholdToKeyPart, armToFillMode, buildEntryKey,
} from "./cohortKeys";

// Backward-compatible re-exports (primitives moved to cohortKeys.js, behaviour
// byte-identical). `thresholdToKeyPart`/`armToFillMode` stay module-private exactly
// as before — they are imported above for internal use only, not re-exported.
export {
    SESSIONS, SESSION_KEYS, CELLS, CELL_KEYS,
    canonicalSession, tradeSessionRaw, canonicalDirection, canonicalStructure,
    tradeCellKey, cohortOf, buildEntryKey,
};
...
// ── Canonicalizers + entry-key building ──────────────────────────────────────
// MOVED to ./cohortKeys.js (4-Layer P0): SESSIONS/CELLS + canonicalSession/
// tradeSessionRaw/canonicalDirection/canonicalStructure/tradeCellKey/cohortOf +
// thresholdToKeyPart/armToFillMode/buildEntryKey. They are imported + re-exported
// at the top of this file (behaviour byte-identical). thresholdToKeyPart and
// armToFillMode are used below by entryKeyFromScenario.

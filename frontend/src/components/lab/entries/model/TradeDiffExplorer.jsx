...
export function TradeDiffExplorer({ exactRows, rawTrades, tradesByMode, activeVariant, selectedModelKey, setSelectedModelKey }) {
    ...
    const columns = [
        ...
        { key: "impact",    label: "Fill Impact", align: "right", render: r => {
            if (r.filled) return <span className="text-[hsl(var(--success))] text-[10px] font-mono">+taken</span>;
            return r.outcome === "Win"
                ? <span className="text-[hsl(var(--danger))] text-[10px] font-mono">missed win</span>
                : <span className="text-[hsl(var(--success))] text-[10px] font-mono">avoided loss</span>;
        }},
    ];
    ...
    if (!nonBaselineRows.length) {
        return (
            <NeonPanel title="Trade Diff Explorer" className="xl:col-span-3" action={<Pill tone="warning">NO MODELS YET</Pill>}>
                <div className="py-6 text-center text-[11px] font-mono text-muted-lab">Import entry model results to explore trade-by-trade differences.</div>
            </NeonPanel>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
                ...
                {nonBaselineRows.map(row => (
                    <button key={row.mode} type="button"
                        ...
                        className={cn(
                            "px-2.5 py-1 text-[10px] font-mono uppercase tracking-wider clip-bevel-sm border transition-colors",
                            selected === row.mode
            ...
            {!diff && (
                <div className="py-4 text-[10.5px] font-mono text-[hsl(var(--warning))]">
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-3">
                        {[["all","All"], ["filled","Filled"], ["skipped","Skipped"]].map(([k, l]) => (
                            <button key={k} type="button" onClick={() => setFilter(k)}
                                className={cn("text-[10px] font-mono uppercase tracking-wider transition-colors",
                        ...
                        <span className="ml-auto text-[10px] font-mono text-muted-lab">

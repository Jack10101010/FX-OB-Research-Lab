...
export function SensitivityPanel({ exactRows }) {
    const penetrationRows = exactRows.filter(r =>
        r.exact && String(r.mode).startsWith("entry_penetration") && isFiniteNumber(r.netR)
    );
    ...
    return (
        ...
        >
            <p className="mb-3 text-[10.5px] font-mono text-muted-lab">Penetration threshold vs. Net R and Fill Rate. Stable plateau = robust threshold. Pick where the curve flattens.</p>
            <ResponsiveContainer width="100%" height={260}>
                <LineChart data={data} margin={{ top: 4, right: 20, bottom: 4, left: 0 }}>
                    ...
                    <XAxis dataKey="label" tick={{ fontSize: 10, fill: "hsl(var(--text-2))", fontFamily: "monospace" }} tickLine={false} />
                    <YAxis yAxisId="left"  tick={{ fontSize: 10, fill: "hsl(var(--text-2))", fontFamily: "monospace" }} tickLine={false} tickFormatter={v => `${v >= 0 ? "+" : ""}${v.toFixed(0)}R`} />
                    <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 10, fill: "hsl(var(--text-2))", fontFamily: "monospace" }} tickLine={false} tickFormatter={v => `${v.toFixed(0)}%`} />
                    <Tooltip

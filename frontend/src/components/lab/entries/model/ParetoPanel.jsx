...
export function ParetoPanel({ exactRows }) {
    ...
    if (data.length < 2) {
        return (
            ...
            >
                <div className="py-8 text-center text-[11px] font-ui text-muted-lab">Import model results to plot the Pareto frontier.</div>
            </NeonPanel>
        );
    }
    ...
    return (
        ...
        >
            <p className="mb-2 text-[10px] font-ui text-muted-lab">X = fill rate, Y = expectancy. Bubble size = |Net R|. Top-right dominates.</p>
            <ResponsiveContainer width="100%" height={260}>
                <ScatterChart margin={{ top: 8, right: 20, bottom: 8, left: 0 }}>
                    ...
                    <XAxis type="number" dataKey="fillPct"    name="Fill %" domain={[0, 100]}
                        tick={{ fontSize: 10, fill: "hsl(var(--text-2))", fontFamily: CHART_NUM_FONT }} tickLine={false}
            ...
            <div className="mt-2 flex flex-wrap gap-2">
                {data.map(d => (
                    <div key={d.mode} className="flex items-center gap-1 text-[9.5px] font-ui text-[hsl(var(--text-2))]">

...
function SampleAdequacyBar({ n }) {
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-mono tabular-nums w-8 text-right" style={{ color }}>
                {n}
            </span>
            <span className="text-[9px] font-mono uppercase tracking-wider w-16" style={{ color }}>
...
function HalfSplitRow({ label, first, second, dim }) {
    ...
    return (
        <tr className="border-t border-[hsl(var(--border-soft)/0.2)]">
            <td className="py-1 px-2 text-[10px] font-mono text-[hsl(var(--text-2))]">{label}</td>
            <td className="py-1 px-2 text-center text-[10px] font-mono tabular-nums text-white">
                {isFiniteNumber(first) ? dim.fmt(num(first)) : "—"}
            </td>
            <td className="py-1 px-2 text-center text-[10px] font-mono tabular-nums text-white">
                {isFiniteNumber(second) ? dim.fmt(num(second)) : "—"}
            </td>
            <td className={cn("py-1 px-2 text-center text-[10px] font-mono tabular-nums", consistent ? "text-[hsl(var(--success))]" : "text-[hsl(var(--warning))]")}>
                {delta != null ? `${delta >= 0 ? "+" : ""}${delta.toFixed(3)}` : "—"}
            </td>
            <td className="py-1 px-2 text-center">
                {delta != null && (
                    <span className={cn("text-[8.5px] font-mono uppercase", consistent ? "text-[hsl(var(--success))]" : "text-[hsl(var(--warning))]")}>
...
function OutlierRow({ label, withOutliers, withoutOutliers, n, removed }) {
    ...
    return (
        <tr className="border-t border-[hsl(var(--border-soft)/0.2)]">
            ...
            <td className="py-1 px-2 text-center text-[9px] font-mono text-muted-lab">
                {removed != null ? `${removed} removed` : "—"}
            </td>
            <td className="py-1 px-2 text-center">
                {dependency != null && (
                    <span className={cn("text-[8.5px] font-mono uppercase", fragile ? "text-[hsl(var(--danger))]" : "text-[hsl(var(--success))]")}>
                ...
                <div className="space-y-2">
                    {exactRows.filter(r => r.exact).map(row => (
                        <div key={row.mode} className="space-y-1">
                            <span className="text-[9.5px] font-mono text-[hsl(var(--text-2))]">{row.label}</span>
                        ...
                        {robustnessScores.map(({ mode, label, score }) => {
                            ...
                            return (
                                <div key={mode} className="flex items-center gap-3">
                                    <span className="text-[10px] font-mono text-[hsl(var(--text-2))] w-28 truncate">{label}</span>
                                    <div className="flex-1 h-2 bg-[hsl(var(--panel-2))] rounded-full overflow-hidden">
                                        <div className="h-full rounded-full" style={{ width: `${score}%`, background: color }} />
                                    </div>
                                    <span className="text-[10px] font-mono tabular-nums w-8 text-right" style={{ color }}>{score}</span>
                                    <span className="text-[9px] font-mono uppercase tracking-wider w-16" style={{ color }}>{rInfo.label}</span>
                                </div>
                            );
                        })}
                    </div>
                ) : (
                    <div className="py-4 text-[10.5px] font-mono text-muted-lab">

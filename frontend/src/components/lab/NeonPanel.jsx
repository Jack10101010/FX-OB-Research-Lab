        ...
        >
            ...
            {(title || action) && (
                <div className={cn("flex items-center justify-between px-4", dense ? "py-2" : "py-3", "border-b border-[hsl(var(--border-soft)/0.6)]")}>
                    {title && (
                        <h3 className="text-[11px] font-mono tracking-[0.22em] uppercase text-muted-lab">
                            {title}
                        </h3>
                    )}
                    {action}
...
export function SectionTitle({ children, className }) {
    return (
        <h4 className={cn("text-[10px] font-mono tracking-[0.22em] uppercase text-muted-lab", className)}>
            {children}
        </h4>
    );
}

...
// ── Backend Directional Scenarios Panel ──────────────────────────────────────
// Standalone panel — always visible when directional results exist, regardless
// of whether AsymmetricEntryPreview is expanded. These are true backend-exported
// split-pass scenarios, NOT post-hoc frontend merges.

function BackendDirectionalScenariosPanel({ bundle }) {
    ...
    if (!directionalScenarios.length) return null;

    return (
        <div className="border border-[hsl(var(--accent-primary)/0.35)] bg-[hsl(var(--accent-primary)/0.03)] clip-bevel-sm">
            {/* Panel header */}
            <div className="px-4 py-3 border-b border-[hsl(var(--border-soft)/0.5)]">
                <div className="flex items-center gap-2 flex-wrap mb-1.5">
                    <span className="text-[10.5px] font-ui uppercase tracking-widest text-[hsl(var(--accent-primary))]">
                ...
                <p className="text-[10px] font-ui text-muted-lab leading-relaxed">
                    True exported directional scenarios from the backtester. Generated by the backtester using directional split-pass execution. Supported for <span className="text-[hsl(var(--text-2))]">one_per_direction</span> and <span className="text-[hsl(var(--text-2))]">allow_multi_position</span>.
                </p>
            </div>
            ...
            <div className="px-4 py-3 space-y-2">
                {directionalScenarios.map(({ storageKey, meta, stats }) => {
                    ...
                    return (
                        <div key={storageKey} className="border border-[hsl(var(--border-soft))] bg-[hsl(var(--panel-2)/0.15)] clip-bevel-sm px-3 py-2.5 space-y-2">
                            {/* Label + badges */}
...
export function SessionLabWorkspace({ trades, bundle }) {
    ...
    return (
        <div className="space-y-5">
            ...
            {/* Asymmetric entry preview (approximate frontend merge) */}
            <AsymmetricEntryPreview bundle={bundle} activeVariant={activeVariant} />

            {/* Backend directional scenarios (true split-pass, always visible) */}
            <BackendDirectionalScenariosPanel bundle={bundle} />
